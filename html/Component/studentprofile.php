...
$db = new Database();
$profile = $db->fetch("select * from studentprofile where User_Id = " . $_GET['id']);
if ($profile != null) {
...
?>
  ...
  <?php endif; ?>
            ...
            <div class="card-body">
                ...
                <divf class="mt-2">
                <button type="button" class="btn btn-primary" id="hireButton">Hire Student</button>
              <script>
                document.getElementById("hireButton").addEventListener("click", function() {
                    var jid = <?php echo $_GET['jid']; ?>;
                    var uid = <?php echo $_GET['id']; ?>;
                    
                    var url = "<?php echo $path; ?>/Function/submit.php?jid=" + jid + "&uid=" + uid + "&submit=hire";
                    window.location.href = url;  // Redirect to the URL with parameters
                });
                </script>
